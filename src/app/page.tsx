...
export default function Home() {
  ...
  const destinations = [
    {
      ...dict.destinations.places[0],
      span: "md:col-span-2 md:row-span-2",
      image:
        "https://d2e5ushqwiltxm.cloudfront.net/wp-content/uploads/sites/236/2024/06/17031656/Khao-Sok-National-Park-1.jpg",
    },
    {
      ...dict.destinations.places[1],
      span: "md:col-span-1 md:row-span-1",
      image:
        "https://images.unsplash.com/photo-1741273229178-8f482aebca4f?q=80&w=1470&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
    },
    {
      ...dict.destinations.places[2],
      span: "md:col-span-1 md:row-span-1",
      image:
        "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcS8hFjKOzwchIVElreXDjzrwKa17jhATARw2w&s",
    },
  ];
      ...
      <HeroSlideshow>
        ...
        <div className="animate-fade-in-up opacity-0 delay-400 w-full max-w-4xl bg-background/95 dark:bg-card/95 backdrop-blur-xl p-2 md:p-4 rounded-3xl shadow-2xl border border-border/50 flex flex-col md:flex-row items-center gap-2 md:gap-4 transition-all hover:shadow-primary/20 hover:-translate-y-1 text-foreground">
          <div className="flex-1 w-full px-4 py-2 border-b md:border-b-0 md:border-r border-border/50 text-left">
            ...
            <input
              type="text"
              placeholder={dict.hero.whereToPlaceholder}
              className="w-full bg-transparent border-none outline-none text-foreground font-medium placeholder:text-muted-foreground/70 focus:ring-0"
            />
          </div>
          <div className="flex-1 w-full px-4 py-2 border-b md:border-b-0 md:border-r border-border/50 text-left">
            <label className="block text-xs font-bold text-muted-foreground uppercase tracking-wider mb-1">
              {dict.hero.travelStyle}
            </label>
          ...
          <a href="/ai-route" className="w-full md:w-auto">
            <Button
              size="lg"
              className="w-full md:w-auto rounded-2xl h-14 px-8 bg-primary hover:bg-primary/90 text-white font-bold text-lg shadow-lg shadow-primary/30 transition-transform active:scale-95 group"
            >
              <svg
      ...
      <section className="relative py-24 px-4 w-full text-white">
        ...
        <div className="relative z-10 max-w-7xl mx-auto md:px-8 animate-fade-in-up opacity-0 delay-300">
          <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-6 text-white">
            <div>
              <h2 className="text-4xl md:text-5xl font-extrabold mb-4 font-heading tracking-tight drop-shadow-md">
            ...
            <a href="/unseen">
              <Button
                variant="outline"
                className="rounded-full px-6 bg-white/10 backdrop-blur-md border-white/50 text-white hover:bg-white hover:text-black transition-all shadow-lg"
              >
                {dict.destinations.exploreBtn}
              </Button>
            </a>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 md:grid-rows-2 gap-4 h-[800px] md:h-[600px]">
            {destinations.map((place, i) => (
              <a
                href="/unseen"
                key={i}
                className={`group relative rounded-3xl overflow-hidden cursor-pointer shadow-md hover:shadow-2xl hover:-translate-y-2 transition-all duration-500 ${place.span}`}
              >
                <img
                  src={place.image}
                  alt={place.title}
